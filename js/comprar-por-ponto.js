...
$(".saveHead").on({
  click: function () {
    $(this).tooltip({
      items: ".saveHead",
      content:
        "URL copied to clipboard!<br>Paste to share your<br> build with a friend. :)",
    });
    $(this).tooltip("open");
...
$(document).on("click", function (event) {
  if (
    !$(event.target).is(".menuCheck") &&
    !$(event.target).is(".hamburger span") &&
    !$(event.target).is(".saveHead")
  ) {
...
function getTotals() {
  ...
  var TotalCost =
    parseInt(PointcostStr) +
    parseInt(PointcostDex) +
    parseInt(PointcostCon) +
    parseInt(PointcostInt) +
    parseInt(PointcostWis) +
    parseInt(PointcostCha);
...
function loadHash(hash) {
  if (initialLoad) {
    ...
    $('select#Race option[data-rchoice="' + hrace + '"]').prop(
      "selected",
      true
    );
    if (hsubrace != "NA") {
      ...
      $(document).ready(
        raceCustomRace(hrstr, hrdex, hrcon, hrint, hrwis, hrcha)
      );
    } else if (
      hrace == "changeling" ||
      hrace == "simichybrid" ||
